...
class AiLeadEmployee::AiProviderLedgerOrchestrationIntent < AiLeadEmployee::AiProviderLedgerRecord
  ...
  def exact_pilot_scope?(pilot) # rubocop:disable Metrics/AbcSize, Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
    ...
    pilot_authorization_id == pilot.id && account_id == pilot.account_id && conversation_id == pilot.conversation_id &&
      observed_control_version == pilot.control_version &&
      ...
      message&.conversation_id == conversation_id && message&.conversation_id == pilot.conversation_id &&
      message&.inbox_id == pilot.inbox_id && message&.message_type&.zero? &&
