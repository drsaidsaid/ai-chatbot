class ActionCableListener < BaseListener
  ...
  def message_created(event)
    ...
    tokens = user_tokens(account, [conversation.assignee].compact) + contact_tokens(conversation.contact_inbox, message)
  ...
  def first_reply_created(event)
    message, account = extract_message_and_account(event)
    conversation = message.conversation
    tokens = user_tokens(account, [conversation.assignee].compact)
  ...
  def conversation_created(event)
    ...
    tokens = user_tokens(account, [conversation.assignee].compact) + contact_inbox_tokens(conversation.contact_inbox)
  ...
  def typing_event_listener_tokens(account, conversation, user)
    ...
    tokens = user_tokens(account, [conversation.assignee].compact) + [conversation.contact_inbox.pubsub_token]
