...
class AiLeadEmployee::ReplyUsageReconciler
  ...
  class << self
    ...
    def manual_release!(usage, platform_app, reason)
      with_finance_evidence(usage, platform_app) do |operator, deliveries|
        raise ArgumentError, 'Confirmed sent evidence prevents allowance release' if
          usage.partially_delivered? || deliveries.any? { |delivery| confirmed_sent?(delivery) }

        AiLeadEmployee::ReplyAllowance.release!(usage: usage, reason: reason, platform_app: operator)
      end
    end
    ...
    def manual_partial_close!(usage, platform_app, reason)
      with_finance_evidence(usage, platform_app) do |operator, deliveries|
        ...
        raise ArgumentError, 'Canonical sent and terminal failure evidence is required for partial closure' unless
          canonical_partial_failure?(usage, deliveries)

        usage.update!(status: :partial_failure_closed, released_at: Time.current, reconciliation_reason: reason,
    ...
    def with_finance_evidence(usage, platform_app)
      usage.account.with_lock do
        ...
        usage.ai_subscription.with_lock do
          ...
          deliveries = usage.whatsapp_outbound_deliveries.order(:id).lock('FOR NO KEY UPDATE').to_a
          Message.where(id: deliveries.map(&:message_id)).order(:id).lock('FOR NO KEY UPDATE').load
          deliveries.each { |delivery| delivery.message.reload }
          yield operator, deliveries
        end
      end
    end
