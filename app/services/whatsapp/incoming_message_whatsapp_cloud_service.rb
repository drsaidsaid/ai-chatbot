...
class Whatsapp::IncomingMessageWhatsappCloudService < Whatsapp::IncomingMessageBaseService
  class MediaUnavailable < StandardError; end
  ...
  def new_provider_message?
    ...
    inbox.messages.find_by(source_id: messages_data.first[:id]).blank?
  end
  ...
  def download_attachment_file(attachment_payload)
    url_response = HTTParty.get(
      ...
      headers: inbox.channel.api_headers, timeout: 10
    )

    # This url response will be failure if the access token has expired.
    inbox.channel.authorization_error! if url_response.unauthorized?

    raise MediaUnavailable unless url_response.success?
