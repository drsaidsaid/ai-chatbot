# R15 — Schedule a bounded permitted follow-up sequence and stop it when circumstances change
...
- [ ] Supersede the original single-follow-up default: at most one contextual reminder and one closing invitation for unfinished permitted conversations, with configurable timing and separate durable attempt identity.
- [ ] Stop sales chasing on refusal, opt-out, payment, booking, closure or human ownership; hesitation alone is not refusal. Transaction confirmations have separate authority and are not marketing follow-ups.
- [ ] Respect actual WhatsApp free-form permission and approved template requirements; never treat every first message as a 72-hour unrestricted window. Suppress if no eligible approved template exists. R26 supplies template management.
- [ ] Preserve R09 attempt lineage/current pointers and do not reset attempts through config revisions, Offer bouncing, retries or reopening.
