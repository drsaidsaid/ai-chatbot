...
- [ ] Evaluate every enabled requirement conjunctively, including multiple lower/upper rules for one field, independent of saved order. Automated sales-call handoff fails closed for exclusions or absent/damaged assessments and requires current met fit, readiness and action eligibility plus explicit positive `sales_call_agreement` evidence. Basic human assistance remains separate.
