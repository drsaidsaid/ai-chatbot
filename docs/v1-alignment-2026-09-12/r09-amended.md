...
- [ ] Evaluate every enabled requirement conjunctively, including multiple lower/upper rules for one field, independent of saved order. Automated sales-call handoff fails closed for exclusions or absent/damaged assessments, requires every configured fit/readiness/action-eligibility requirement to be met, and requires explicit positive `sales_call_agreement` evidence. A canonical `not_required` dimension is valid only when the current Offer configures no enabled requirement or enabled required question for it. Basic human assistance remains separate.
