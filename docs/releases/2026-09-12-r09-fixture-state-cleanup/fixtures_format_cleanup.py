...
report=json.loads((tmp/'fixtures-combined-lint-before.json').read_text())
...
e=os.environ.copy();e['GIT_INDEX_FILE']=str(tmp/'fixtures-format.index')
...
before=snapshot();git('update-ref','refs/r09/fixtures-format-before-source-20260912',before,'0'*40)
backup=tmp/'fixtures-format-before';backup.mkdir(exist_ok=True)
for path in paths:
...
command=['/Users/ghalyasaid/.rbenv/versions/3.4.4/bin/bundle','exec','rubocop','--force-exclusion','--only',','.join(cops),'-a',*paths,'--format','json','--out',str(tmp/'fixtures-format-autocorrect-results.json')]
with (tmp/'fixtures-format-autocorrect.log').open('w') as log:result=subprocess.run(command,env=env,stdout=log,stderr=subprocess.STDOUT)
...
after=snapshot();git('update-ref','refs/r09/fixtures-format-after-source-20260912',after,'0'*40)
...
(tmp/'fixtures-format-proof.json').write_text(json.dumps(proof,indent=2)+'\n')
