...
for name,args in [('vue',vue),('rails',ruby)]:
 ...
 (root/f'tmp/r09/ui-checked-{name}-invocation.json').write_text(json.dumps(args,indent=2)+'\n')
 with (root/f'tmp/r09/ui-checked-{name}.log').open('w') as log:result=subprocess.run(args,env=env,stdout=log,stderr=subprocess.STDOUT)
 ...
 (root/'tmp/r09/ui-checked-verification.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(run),flush=True)
