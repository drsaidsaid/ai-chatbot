...
- The focused suite below passed on isolated local PostgreSQL: **38 examples, 0 failures**.
...
bundle exec rspec spec/requests/platform/pilot_authorizations_spec.rb \
  spec/models/ai_lead_employee/pilot_authorization_spec.rb \
...
The test set covers owner-approval input, activation permission/key-revision drift, exact persisted intent scope, two real PostgreSQL connections contending for the final pilot attempt, failed-attempt cost uncertainty, preservation of an operator stop, final known-cost dispatch, intent recording, and handoff-alert suppression. The Platform API request specs exercise the current-key verifier through a stubbed OpenRouter response; no standalone verifier spec exists.
