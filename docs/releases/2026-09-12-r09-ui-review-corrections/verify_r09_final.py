...
root=pathlib.Path.cwd();source=json.loads((root/'tmp/r09/r09-verified-identity.json').read_text())['source_tree']
def snapshot():
 e=os.environ.copy();e['GIT_INDEX_FILE']=str(root/'tmp/r09/r09-verified-verify.index')
 def git(*a):return subprocess.check_output(['git',*a],env=e).decode().strip()
 git('read-tree','HEAD');git('add','-A','--','.');return git('write-tree')
vue=['corepack','pnpm','exec','vitest','run','app/javascript/dashboard/routes/dashboard/settings/aiLeadEmployee/specs/OfferConfigurationPanel.spec.js','app/javascript/dashboard/routes/dashboard/owned/specs/LeadsDirectoryPage.spec.js','app/javascript/dashboard/components/qualification/specs','app/javascript/dashboard/routes/dashboard/conversation/specs/AIEmployeeControlPanel.spec.js','app/javascript/dashboard/routes/dashboard/settings/aiLeadEmployee/specs/OwnedSettingsLayout.spec.js','app/javascript/dashboard/routes/dashboard/owned/specs/AiProviderSettingsPage.spec.js','app/javascript/dashboard/components-next/sidebar/specs/aiLeadEmployeeNavigation.spec.js','--maxWorkers=1','--minWorkers=1','--reporter=default','--reporter=json','--outputFile=tmp/r09/r09-verified-vue-results.json']
ruby=['/Users/ghalyasaid/.rbenv/versions/3.4.4/bin/bundle','exec','rspec',*json.loads((root/'tmp/r09/required-r09-rails-paths.json').read_text()),'--format','progress','--format','json','--out','tmp/r09/r09-verified-rails-results.json']
rubocop=['/Users/ghalyasaid/.rbenv/versions/3.4.4/bin/bundle','exec','rubocop','--force-exclusion',*json.loads((root/'tmp/r09/required-r09-ruby-lint-paths.json').read_text()),'--format','json','--out','tmp/r09/r09-verified-rubocop-results.json']
eslint=['corepack','pnpm','exec','eslint',*json.loads((root/'tmp/r09/required-r09-eslint-paths.json').read_text()),'--format','json','--output-file','tmp/r09/r09-verified-eslint-results.json']
build=['corepack','pnpm','exec','vite','build']
report={'source_tree':source,'runs':[]}
for name,args in [('vue',vue),('rails',ruby),('rubocop',rubocop),('eslint',eslint),('build',build)]:
 ...
 (root/f'tmp/r09/r09-verified-{name}-invocation.json').write_text(json.dumps(args,indent=2)+'\n')
 with (root/f'tmp/r09/r09-verified-{name}.log').open('w') as log:result=subprocess.run(args,env=env,stdout=log,stderr=subprocess.STDOUT)
 ...
 (root/'tmp/r09/r09-verified-verification.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(run),flush=True)
