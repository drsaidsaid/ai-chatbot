...
RSpec.describe AiLeadEmployee::SubscriptionAlertDeliveryService do
  ...
  it 'keeps exhausted alerts failed when duplicate subscription jobs bypass the recovery scope' do
    channel = create(:channel_whatsapp, provider: 'whatsapp_cloud', sync_templates: false, validate_provider_config: false)
    account = channel.account
    create(:user, account: account, role: :administrator,
                  custom_attributes: { 'whatsapp_alert_phone' => '+255700123456' })
    ...
    delivery.update!(state: :failed, failure_code: 'provider_rejected', attempts: Whatsapp::OutboundDelivery::MAX_CLAIM_ATTEMPTS)

    2.times { described_class.new(alert: alert.reload).perform }

    expect(delivery.reload).to have_attributes(state: 'failed', attempts: Whatsapp::OutboundDelivery::MAX_CLAIM_ATTEMPTS)
    expect(account.messages.where(id: message.id).count).to eq(1)
    expect(delivery.retry_subscription_alert!).to be(false)
  end
